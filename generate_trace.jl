...
#   global eval_sizes = reshape([2 for h in 1:6], 1, :)
  ...
  include("params/params_N25_S34_Networktest_25_5.jl")

  include("environments/tensor_alphazeroRTN_greedy_competing.jl")
  ...
  trace = play_game(gspec, p; flip_probability=0., id=1, worker_id=1, reset_every=nothing)
  print(last(trace.states).greedy_result, first(trace.states).greedy_result, "\n")  
  print(last(trace.states).history, last(trace.states).total_reward*last(trace.states).greedy_result, "\n" )
  push!(rewards, last(trace.states).total_reward*last(trace.states).greedy_result)
