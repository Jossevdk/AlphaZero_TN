...
mutable struct tensor_environment
  ...
  function tensor_environment(num_workers, N, S, eval_mode)
    if eval_mode
      ...
      new_best_result = [nothing for i in 1:num_workers]
      new(new_A, sizes, new_best_result)
    else
...
global tenv = tensor_environment(self_play.sim.num_workers, N, S, env_params.eval_mode)
...
function GI.init(::GameSpec; id::Union{Int64,Nothing}=0, worker_id::Union{Int64,Nothing}=1, reset_every::Union{Int64,Nothing}=nothing, network_name::Union{String,Nothing}=nothing)
  ...
  elseif network_name == "contender" || network_name == "baseline" || network_name == "benchmark"
    eval_mode = true
  end
    ...
    if !eval_mode && !isnothing(id) && !isnothing(reset_every)
      if id % reset_every == 0
        ...
        if env_params.use_baseline
          ...
          tenv.best_result[worker_id] = -einexpr(Greedy(metric=flops), path)[2]
        end
  ...
  ef = vcat(tenv.sizes[worker_id], fill(0.0f0, 11, S))
  attr = ef[1:4, :]

  fg = FeaturedGraph(A, ef=ef, nf=nf)

  edge_centrality = edge_betweenness_centrality(fg.graph)
  for edge in GraphSignals.edges(fg)
    ...
    attr[4, e] = edge_centrality[(n1, n2)]
  end
  ...
  return GameEnv(fg, attr, 0.0, 0.0, collect(1:env_params.S), amask, false, eval_mode, history)
...
function GI.clone(env::GameEnv)
  ...
  return GameEnv(copy(env.fg), copy(env.attr), copy(env.total_reward), copy(env.reward), copy(env.o_edge), copy(env.amask), copy(env.finished), copy(env.eval_mode), history)
end
...
function GI.play!(env::GameEnv, action; worker_id=1)
  ...
  if env.finished == false
    ...
    edg = [e for e in GraphSignals.edges(env.fg)][1:GraphSignals.ne(env.fg)]
    if update
      edge_centrality = edge_betweenness_centrality(env.fg.graph)
  ...
  lock(lock_) do
    if !env.eval_mode && env_params.use_feas_act && env.finished
  ...
  lock(lock_) do
    if !env.eval_mode && env_params.use_feas_act && env.finished == false
      if !isnothing(tenv.best_result[worker_id])
        if env.total_reward - minimum(attr[2, :]) < tenv.best_result[worker_id]
          env.finished = true
          alpha = 0.5
          ...
          penalty = abs(minimum(attr[2, :])) - future_term
        end
      end
      env.reward -= penalty
...
GI.current_state(env::GameEnv) = (fg=copy(env.fg), attr=copy(env.attr), total_reward=copy(env.total_reward), reward=copy(env.reward), o_edge=copy(env.o_edge), amask=copy(env.amask), finished=copy(env.finished), eval_mode=copy(env.eval_mode), history=copy(env.history))
