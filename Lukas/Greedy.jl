...
function einexpr(config::Greedy, path::EinExpr{L}, sizedict::Dict{L}) where {L}
    ...
    while nargs(path) > 1 && length(queue) > 1
        # choose winner
        
        w, winner = config.choose(queue)
...
function einexpr(config::Greedy, path::SizedEinExpr)  
    p = einexpr(config, path.path, path.size)

    return SizedEinExpr(p[1], path.size),  p[2]
end
