...
function GI.init(::GameSpec)
  ...
  N = env_params.N #maximum(con_ary)
  S = env_params.S
  ...
  while LightGraphs.is_connected(graph) == false
    graph = LightGraphs.SimpleGraphs.erdos_renyi(N, S)
  end
  ...
  for edge in GraphSignals.edges(fg)
    ...
    #greedy
    ef[3, e] = prod([ef[1, i] for i in indices1]) + prod([ef[1, i] for i in indices2]) - ef[2, e]/ef[1, e]
  end
  ...
  return GameEnv(fg,  0.0, collect(1:env_params.S), amask, false, history)
...
function GI.play!(env::GameEnv, action)
  ...
  edge_size = edge_feature(env.fg)[1, action]
  ...
  if env.finished == false
    ...
    for i in update_e
      ...
      ef[e][2] = prod([ef[i][1] for i in indices])/ef[i][1]
      #greedy
      ef[e][3] = prod([ef[i][1] for i in indices1]) + prod([ef[i][1] for i in indices2]) - ef[i][2]/ef[i][1]
