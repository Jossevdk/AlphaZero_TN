...
abstract type AbstractGraphNet end
...
to_singletons(x) = hasmethod(size, Tuple{typeof(x)}) ? reshape(x, size(x)..., 1) : x
from_singletons(x) = hasmethod(size, Tuple{typeof(x)}) ? reshape(x, size(x)[1:end-1]) : x
...
function evaluate_fg(nn::AbstractNetwork, state)
  ...
  net_output = forward_normalized(nn, fgnet, anet)
  p = from_singletons(convert_output_tuple(nn, net_output)[1])
  v = from_singletons(convert_output_tuple(nn, net_output)[2])
  p, v, _ = from_singletons.(convert_output_tuple(nn, net_output))
  return (p[actions_mask], v[1])
end
...
function evaluate_batch(nn::AbstractNetwork, batch)
  ...
  gspec = game_spec(nn)
  if GI.hasgraph(gspec)
    return evaluate_batch_fg(nn, batch)
  end
