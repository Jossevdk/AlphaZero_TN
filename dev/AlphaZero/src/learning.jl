...
function convert_samples(
  ...
  W = f32(Flux.batch([e.w for e in ces]))
  X = [e.x for e in ces]
  A = f32(Flux.batch([e.a for e in ces]))
  P = f32(Flux.batch([e.p for e in ces]))
  V = f32(Flux.batch([e.v for e in ces]))
  return (; W, X, A, P, V)
end
